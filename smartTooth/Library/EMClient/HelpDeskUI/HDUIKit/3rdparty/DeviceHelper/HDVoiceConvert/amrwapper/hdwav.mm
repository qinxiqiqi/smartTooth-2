...
void HDWavWriter::writeString(const char *str) {
	fputc(str[0], wav);
	fputc(str[1], wav);
	fputc(str[2], wav);
	fputc(str[3], wav);
...
void HDWavWriter::writeInt32(int value) {
...
void HDWavWriter::writeInt16(int value) {
	fputc((value >> 0) & 0xff, wav);
	fputc((value >> 8) & 0xff, wav);
}

void HDWavWriter::writeHeader(int length) {
    writeString("RIFF");
...
HDWavWriter::HDWavWriter(const char *filename, int sampleRate, int bitsPerSample, int channels)
{
...
void HDWavWriter::writeData(const unsigned char* data, int length) {
	if (wav == NULL)
		return;
