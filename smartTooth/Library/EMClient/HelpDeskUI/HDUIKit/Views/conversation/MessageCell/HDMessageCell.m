...
+ (CGFloat)getArticleCellHeight:(NSArray *)subs {
    ...
    if (arr.count == 1) {
        ...
        if (item.digest.length == 0 || kDigistFontSize == 0) {
            
        }
        CGRect rect = [item.digest boundingRectWithSize:size options:NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:kDigistFontSize]} context:nil];
        CGFloat digistH = rect.size.height;
