...
@interface QXClinicHomeController ()<QXClinicHomeHeaderDelegate,UIScrollViewDelegate,SPPageMenuDelegate,WKNavigationDelegate>
@property (nonatomic, strong) QXClinicHomeHeader *headerView;
...
@implementation QXClinicHomeController
...
- (void)initView {
    ...
    // 添加2个子控制器
//    QXServerListController *serverListVC = [[QXServerListController alloc] init];
//    serverListVC.view.frame = CGRectMake(0, 0, ScreenWidth, ScreenHeight);
//    serverListVC.topMargin = self.pageMenu.bottom;
    QXDoctroListController *doctorListVC = [[QXDoctroListController alloc] init];
    doctorListVC.dataArray = [_clinicModel.doctors mutableCopy];
    doctorListVC.fromType = 1;
    doctorListVC.view.frame = CGRectMake(0, 0, ScreenWidth, ScreenHeight);
    doctorListVC.topMargin = self.pageMenu.bottom;
    //[self addChildViewController:serverListVC];
    [self addChildViewController:doctorListVC];
...
- (void)setupNaviBar {
    ...
    titleLabel.text = @"诊所主页";
    titleLabel.textColor = [UIColor whiteColor];
    titleLabel.text = _clinicModel.cname;
    [naviBar addSubview:titleLabel];
    
    UIButton *backItem = [[UIButton alloc] initWithFrame:CGRectMake(0, kStatusBarHeight, 52, 44)];
    ...
    UIButton *rightBtn = [[UIButton alloc] initWithFrame:CGRectMake(ScreenWidth - 40, backItem.top, 40, backItem.height)];
    [rightBtn setImage:[UIImage imageNamed:@"share-white"] forState:UIControlStateNormal];
    //[naviBar addSubview:rightBtn];
    
    UIButton *collectBtn = [[UIButton alloc] initWithFrame:CGRectMake(rightBtn.left - 40, backItem.top, 40, backItem.height)];
    [collectBtn setImage:[UIImage imageNamed:@"collection-icon"] forState:UIControlStateNormal];
    //[naviBar addSubview:collectBtn];
}
...
{
    QXWebViewController *wewbView = [[QXWebViewController alloc] init];
    wewbView.navigationItem.title = clinic.cname;
    wewbView.requestUrl = clinic.curl;
    [self.navigationController pushViewController:wewbView animated:YES];
}
...
{
    QXPhotoBrowserController *photoBrowserVC = [[QXPhotoBrowserController alloc] init];
    photoBrowserVC.photoList = clinic.photos;
...
- (SPPageMenu *)pageMenu {
    ...
    if (!_pageMenu) {
        ...
        [_pageMenu setItems:@[@"医师团队"] selectedItemIndex:0];
...
- (QXClinicHomeHeader *)headerView
{
    if (!_headerView) {
        _headerView = [[QXClinicHomeHeader alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 0)];
        _headerView.height = _headerView.headerHeight;
        _headerView.delegate = self;
        self.originalHeaderImgRect = _headerView.backImgView.frame;
...
- (void)getClinicDetailData {
    NSMutableDictionary *param = [NSMutableDictionary dictionary];
    param[@"clinic_id"] = self.clinicModel.ID;
    [[UFHttpRequest defaultHttpRequest] postRequest:API_CLINIC_DETAIL alertView:nil parameters:param success:^(id  _Nullable responseObject) {
        if ([responseObject[@"code"] integerValue] == 0) {
            self.clinicModel = [QXClinicModel mj_objectWithKeyValues:responseObject[@"data"]];
            self.headerView.clinicModel = self.clinicModel;
        }
